...
- YouTube Data API v3 직접 업로드 (Selenium 불필요)
- 15채널 동시 관리
- Flask 대시보드
- 실행: python app.py
"""
...
def get_youtube_service(ch_id):
    ...
    if not token_path.exists():
        log(f"[{ch_id}] 토큰 없음 → auth_setup.py 먼저 실행!", "err")
        return None
    try:
...
ALL_CHANNELS = list(CHANNELS.keys())  # ch01 ~ ch15

def auto_round(round_name):
    """아침/점심/저녁 자동 업로드 - 15채널 순서대로"""
