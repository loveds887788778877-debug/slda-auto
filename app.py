...
HASHTAGS = {
    "ch01": ["#명언","#동기부여","#힐링","#오늘의말한마디","#긍정에너지","#자기계발","#성장","#좋은글","#공감","#인생명언"],
    "ch02": ["#감성","#음악","#무드","#플레이리스트","#힐링음악","#새벽감성","#로파이","#감성팝","#인디음악","#뮤직"],
    "ch03": ["#운동","#헬스","#홈트","#다이어트","#근성장","#유산소","#스트레칭","#건강","#헬린이","#바디프로필"],
    "ch04": ["#ASMR","#수면","#백색소음","#릴렉스","#힐링","#숙면","#집중력","#공부asmr","#자연소리","#수면음악"],
    "ch05": ["#강아지","#고양이","#반려동물","#귀여운","#댕댕이","#냥이","#펫스타그램","#동물영상","#멍스타그램","#고양이스타그램"],
    "ch06": ["#리뷰","#추천","#꿀템","#솔직리뷰","#언박싱","#가성비","#득템","#인생템","#구매후기","#써보고말함"],
    "ch07": ["#이슈","#정보","#트렌드","#핫이슈","#꿀정보","#생활정보","#실시간이슈","#화제","#오늘의이슈","#SNS이슈"],
    "ch08": ["#뷰티","#메이크업","#스킨케어","#화장품","#올리브영","#피부관리","#데일리메이크업","#선크림","#뷰티팁","#뷰티루틴"],
    "ch09": ["#맛집","#요리","#음식","#먹방","#레시피","#홈쿡","#간단레시피","#자취요리","#혼밥","#디저트"],
    "ch10": ["#영화","#드라마","#추천","#넷플릭스","#영화리뷰","#드라마추천","#결말해석","#명작","#영화추천","#OTT"],
    "ch11": ["#패션","#코디","#룩북","#오오티디","#데일리룩","#스타일","#무신사","#에이블리","#트렌드","#빈티지"],
    "ch12": ["#드라마","#연예","#인기드라마","#넷플릭스드라마","#드라마리뷰","#신작드라마","#로맨스드라마","#kdrama","#한드","#드라마추천"],
    "ch13": ["#여행","#국내여행","#해외여행","#여행vlog","#제주도","#부산","#서울","#여행추천","#힐링여행","#여행일상"],
    "ch14": ["#퀴즈","#상식","#두뇌트레이닝","#문제풀기","#역사퀴즈","#상식퀴즈","#도전","#지식","#공부","#학습"],
    "ch15": ["#밈","#공감","#유머","#웃긴영상","#공감짤","#현실공감","#직장인공감","#학생공감","#재미있는영상","#웃음"],
}

def get_hashtags(ch_id):
    tags = HASHTAGS.get(ch_id, HASHTAGS["ch07"])
    return " ".join(tags) + " #shorts #쇼츠"
...
def get_kling_video(ch_id, script):
    ...
    try:
        ...
        res = requests.post("https://api.klingai.com/v1/videos/image2video", headers=headers, json=data, timeout=30)
        ...
        for i in range(18):
            time.sleep(10)
            check = requests.get(f"https://api.klingai.com/v1/videos/image2video/{task_id}", headers=headers, timeout=15)
        ...
        if p.exists():
            try:
                ...
                if creds.expired and creds.refresh_token:
                    ...
                    with open(p, "wb") as f:
                        pickle.dump(creds, f)
                    log(f"[{ch_id}] 토큰 갱신 완료!", "ok")
                if not creds.valid:
                    log(f"[{ch_id}] 토큰 무효 → 재인증 필요", "err")
                    if ch_id not in expired_tokens:
                        expired_tokens.append(ch_id)
                    return None
                return build("youtube", "v3", credentials=creds)
            except Exception as e:
                err_str = str(e)
                if "invalid_grant" in err_str or "Token has been expired or revoked" in err_str:
                    log(f"[{ch_id}] 토큰 만료/취소 → 재인증 필요!", "err")
                    if ch_id not in expired_tokens:
                        expired_tokens.append(ch_id)
                else:
...
def upload_youtube(ch_id, video_path, content):
    ...
    try:
        ...
        hashtag_str = get_hashtags(ch_id)
...
@app.route("/logs")
def get_logs():
    return jsonify({"logs": upload_log[-100:], "stats": upload_stats, "pipeline": pipeline_status, "expired": expired_tokens})
