"""
슬다 자동화 v12.1 - 영상 처리 오류 수정판
- ffmpeg 자막 경로 오류 수정 (Railway Linux 호환)
- 빈 파일 업로드 방지 (크기 체크 추가)
- Pexels 영상 없을 때 색상 배경으로 대체
- 토큰 매핑 ch01_token.pickle / token_ch01.pickle 둘 다 지원
"""
...
def get_pexels_video(ch_id):
    ...
    try:
        ...
        if video_path.stat().st_size < 10000:
            log(f"[{ch['name']}] Pexels 파일 너무 작음, 재시도", "warn")
            return None
...
def make_color_video(ch_id, duration=30):
    """✅ 추가: Pexels 실패 시 색상 배경 영상 생성"""
    colors = ["#1a1a2e", "#16213e", "#0f3460", "#533483", "#2d6a4f", "#1b4332"]
    color = random.choice(colors).replace("#","")
    ...
    try:
        ...
        if output_path.exists() and output_path.stat().st_size > 1000:
            log(f"[{ch_id}] 색상 배경 영상 생성 완료", "ok")
            return output_path
    except Exception as e:
        log(f"[{ch_id}] 색상 배경 생성 실패:{e}", "err")
    return None
...
def make_voice(script, ch_id):
    ...
    try:
        ...
        if audio_path.stat().st_size < 1000:
            log(f"[{ch_id}] 음성 파일 너무 작음", "warn")
            return None
        return audio_path
    except Exception as e:
        log(f"[{ch_id}] 음성 생성 오류:{e}", "err")
        return None
...
def make_video(ch_id, base_video, audio_path, subtitle_path):
    ...
    # ✅ 수정: 자막 없이 먼저 시도 (자막이 Railway에서 경로 오류 유발)
    cmd_no_sub = [
        FFMPEG_EXE, "-y",
        "-i", str(base_video),
        "-i", str(audio_path),
    ...
    try:
        log(f"[{ch_id}] ffmpeg 합성 시작...", "info")
        result = subprocess.run(cmd_no_sub, capture_output=True, timeout=180)

        # ✅ 수정: 출력 파일 크기로 성공 여부 판단
        if output_path.exists() and output_path.stat().st_size > 50000:
            log(f"[{ch_id}] 영상 합성 완료! ({output_path.stat().st_size//1024}KB)", "ok")
            return output_path
        else:
            stderr = result.stderr.decode("utf-8", errors="ignore")[-500:]
            log(f"[{ch_id}] ffmpeg 실패: {stderr}", "err")
            return None

    except subprocess.TimeoutExpired:
        log(f"[{ch_id}] ffmpeg 타임아웃 (180초 초과)", "err")
        return None
    except Exception as e:
        log(f"[{ch_id}] ffmpeg 오류:{e}", "err")
        return None

def get_youtube_service(ch_id):
    # ✅ 수정: 두 형식 모두 지원 (ch01_token.pickle / token_ch01.pickle)
    ch_num = ch_id[2:]  # "ch01" -> "01"
    candidates = [
    ...
    log(f"[{ch_id}] 토큰 없음! (찾은 경로:{[str(c) for c in candidates]})", "err")
    return None

def upload_youtube(ch_id, video_path, content):
    # ✅ 수정: 업로드 전 파일 크기 반드시 확인
    if not Path(video_path).exists():
        log(f"[{ch_id}] 업로드 취소: 파일 없음", "err")
        return False
    size = Path(video_path).stat().st_size
    if size < 50000:
        log(f"[{ch_id}] 업로드 취소: 파일 너무 작음 ({size}bytes)", "err")
        return False
...
def run_pipeline(channel_ids, script=None):
    ...
    for i,ch_id in enumerate(channel_ids,1):
        ...
        try:
            ...
            # ✅ 수정: Pexels 실패 시 색상 배경으로 자동 대체
            base_video = get_pexels_video(ch_id)
            if not base_video:
                log(f"[{ch['name']}] Pexels 없음 → 색상 배경 사용", "warn")
                base_video = make_color_video(ch_id, duration=30)
            if not base_video:
                log(f"[{ch['name']}] 영상 생성 완전 실패!", "err")
